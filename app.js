      ...
      import { UI_MODE_VALUES, TAB_MODE_LABELS, TAB_CONTEXT_HINTS, normalizeUiMode } from "./ui-mode-constants.js";
      ...
      let computeAdvancedPricing, deriveHistoricalVarianceSamples;
      ...
      const els = {
        ...
        pricingHudProjectCompact: $("pricingHudProjectCompact"),
        pricingHudProjectValue: $("pricingHudProjectValue"),
        dailyRate: $("dailyRate"),
      ...
      function setInputsFromState(s) {
        ...
        if (els.proposalMode) {
          els.proposalMode.checked = !!s.proposalMode;
        }
        if (els.pdfInternalFormat) {
          const v = (s.pdfInternalFormat === "compact" ? "compact" : "complete");
          els.pdfInternalFormat.value = v;
        }
        if (els.advancedMode) {
          els.advancedMode.checked = !!s.advancedMode;
        }
        if (els.enableMonteCarlo) {
          els.enableMonteCarlo.checked = !!s.enableMonteCarlo;
        }
        ...
        if (els.scopeVolatility) {
          const allowed = ["low", "medium", "high"];
          const value = allowed.includes(s.scopeVolatility) ? s.scopeVolatility : "medium";
          els.scopeVolatility.value = value;
        }
        if (els.complexidadeProjeto) {
          const allowed = ["baixa", "media", "alta"];
          const value = allowed.includes(s.complexidadeProjeto) ? s.complexidadeProjeto : "media";
          els.complexidadeProjeto.value = value;
        }
        if (els.prazoEntrega) {
          const allowed = ["flexivel", "normal", "curto", "urgente"];
          const value = allowed.includes(s.prazoEntrega) ? s.prazoEntrega : "normal";
          els.prazoEntrega.value = value;
        }
        if (els.criticidadeNegocio) {
          const allowed = ["baixa", "media", "alta"];
          const value = allowed.includes(s.criticidadeNegocio) ? s.criticidadeNegocio : "media";
          els.criticidadeNegocio.value = value;
        }
        if (els.envolvimentoCliente) {
          const allowed = ["baixo", "medio", "alto"];
          const value = allowed.includes(s.envolvimentoCliente) ? s.envolvimentoCliente : "medio";
          els.envolvimentoCliente.value = value;
        }
        if (els.impactoNoNegocio) {
          const allowed = ["baixo", "medio", "alto", "critico"];
          const value = allowed.includes(s.impactoNoNegocio) ? s.impactoNoNegocio : "medio";
          els.impactoNoNegocio.value = value;
        }
      ...
      function isPlaceholderText(value) {
        ...
        return t === "" || t === PLACEHOLDER || t === "-" || t === "–";
      }

      function safeText(node, text) {
        if (!node) return;
        updateResultNode(node, () => {
      ...
      function safeMoney(node, text) {
        if (!node) return;
        const parts = splitMoneyParts(text);
        updateResultNode(node, () => {
          if (!parts) {
            node.textContent = text;
            return;
          ...
          node.textContent = "";
          const symbolClass = parts.trailing ? "money-symbol trailing" : "money-symbol";
          const amt = createEl("span", "money-amount", parts.amount);
          const sym = createEl("span", symbolClass, parts.symbol);
          if (parts.trailing) {
      ...
      function renderScenariosComparison() {
        ...
        if (!scenarios.A || !scenarios.B) {
          els.scenarioDelta.textContent = "Salve os dois cenários para ver as diferenças.";
          return;
        }
      ...
      function saveScenario(slot) {
        ...
        if (!payload) {
          showToast("Não foi possível salvar: ajuste os dados para um cálculo válido.");
          return;
        }
        const scenarios = loadScenarios();
      ...
      function exportCsv() {
        ...
        const flat = {
          ...
          ...payload.inputs,
          revenueTarget: payload.outputs.revenueTarget,
          hourly: payload.outputs.hourly,
          daily: payload.outputs.daily,
          billableHours: payload.outputs.billableHours,
      ...
      async function sendToEndpoint(kind) {
        ...
        if (!validation.ok) {
          if (validation.reason === "HTTP_EXTERNAL_BLOCKED") {
            showToast("Endpoint bloqueado: para host externo use HTTPS. HTTP só é permitido em localhost/127.0.0.1.");
          } else {
            showToast("Endpoint inválido. Revise a configuração.");
          }
          return;
        }
        const payload = buildExportPayload();
      ...
      async function triggerAppInstall() {
        if (!deferredInstallPrompt) {
          showToast("Instalação não disponível neste navegador.");
          return;
        }
      ...
      function applyBranding() {
        ...
        if (metaDescription) {
          metaDescription.setAttribute(
            "content",
            `${BRAND_SUBTITLE} com proposta em PDF, auditoria e governança de dados.`
          );
        }
        ...
        const ctaMap = {
          btnExportConfig: "Exportar configuração",
          btnImportConfig: "Importar configuração",
          btnCopyProposal: "Copiar proposta",
          btnCopyJustification: "Copiar justificativa técnica",
      ...
      function applyButtonHelp() {
        const helpMap = {
          btnSaveScenarioA: "Salva as entradas e resultados atuais como Cenário A.",
          btnSaveScenarioB: "Salva as entradas e resultados atuais como Cenário B.",
          btnLoadScenarioA: "Carrega o Cenário A nas entradas atuais.",
          btnLoadScenarioB: "Carrega o Cenário B nas entradas atuais.",
          btnOpenIntegrationSettings: "Abre configuração de endpoints para integrações.",
          ...
          enableMonteCarlo: "Ativa simulação estocástica quando houver histórico suficiente.",
          advancedMode: "Alterna entre cálculo essencial e precificação avançada.",
          btnCopyJustification: "Copia os blocos de justificativa para proposta comercial.",
          btnInsertProposalJustification: "Alterna entre o texto curto e a versão com justificativa técnica.",
        };
      ...
      function setupTabs() {
        ...
        function activateTab(index, fromClick, skipToast) {
          ...
          currentUiMode = UI_MODE_VALUES[index] ?? "essencial";
          ...
          if (hintEl) hintEl.textContent = TAB_CONTEXT_HINTS[currentUiMode] ?? "";
        ...
        if (premiumLock() && els.btnPremiumOffer) {
          els.btnPremiumOffer.textContent = "Conhecer Premium 🔒";
        ...
        const initialIndex = UI_MODE_VALUES.indexOf(currentUiMode);
        activateTab(initialIndex >= 0 ? initialIndex : 0, false, true);
      }
        ...
        if (els.btnWizardNext) {
          els.btnWizardNext.addEventListener("click", () => {
            ...
            if (wizardCurrentStep < 3 && !r.ok && wizardCurrentStep === 1) {
              showToast("Faltam campos para cálculo completo. Avançar mesmo assim.");
            }
      ...
      function buildJustificationClipboardText(s, n) {
        ...
        parts.push(n.justification.caveat || "Estimativa sujeita à execução e variáveis operacionais.");
      ...
      function updateUI() {
        ...
        // normalize inputs (clamp) — only when elements exist
        ...
        const isProposalMode = !!s.proposalMode;
        const canComputeProposal = !!(r && r.ok && s.projectHours > 0 && r.projectNet != null);
        const canEstimatePrazo =
          s.projectHours > 0 &&
          Number(s.hoursPerDay) > 0 &&
          Number(s.utilization) > 0;
        const prazoDias = canEstimatePrazo
          ? Math.ceil((s.projectHours / Math.max(1, s.hoursPerDay)) * (100 / Math.max(1, s.utilization)))
          : null;

        if (els.resultError) {
          if (isProposalMode && !canComputeProposal) {
            const baseMsg =
              r && r.error && r.error.message
                ? r.error.message
                : "Dados insuficientes. Preencha as entradas essenciais e as horas do projeto.";
            els.resultError.textContent = `Modo Proposta: ${baseMsg}`;
            els.resultError.classList.remove("hidden");
          } else if (isProposalMode && canComputeProposal) {
            els.resultError.classList.add("hidden");
            els.resultError.textContent = "";
          } else if (pricingCtx.warning) {
        ...
        if (els.pricingHudSustainableValue) safeMoney(els.pricingHudSustainableValue, r.ok && r.revenueTarget != null ? fmtMoney(r.revenueTarget, curr) : "—");
        if (els.pricingHudFloorValue) safeMoney(els.pricingHudFloorValue, "—");
        if (els.pricingHudIdealValue) safeMoney(els.pricingHudIdealValue, "—");
        ...
        safeText(
          els.billableHours,
          r.billableHours != null ? `${fmtNumber(r.billableHours, 1)} h` : "—"
        );

        safeMoney(els.revenueTarget, r.ok && r.revenueTarget != null ? fmtMoney(r.revenueTarget, curr) : "—");
        safeText(
          els.revenueBreakdown,
          `Base: ${fmtMoney(r.baseNeed, curr)} • Total: ${fmtNumber(s.taxRate, 1)}% + ${fmtNumber(
            s.profitMargin,
            1
        ...
        if (!r.ok && r.error) {
          ...
          safeText(
            els.stepProfit,
            `${fmtMoney(r.baseNeed, curr)} ÷ (1 − ${fmtNumber(r.totalPercent, 1)}%) = ${fmtMoney(r.revenueTarget, curr)}`
          );
        } else {
          safeText(els.stepProfit, "—");
        }

        safeText(
          els.stepHours,
          `Semanas úteis/ano: ${fmtNumber(r.workingWeeks, 1)} • Horas/mês: ${fmtNumber(r.hoursPerMonth, 1)} • Faturáveis: ${fmtNumber(
            r.billableHours,
            1
          )}`
        );
        ...
        if (els.pricingHudProjectCompact && els.pricingHudProjectValue) {
          els.pricingHudProjectCompact.classList.toggle("hidden", !projectOk);
          if (projectOk) safeMoney(els.pricingHudProjectValue, fmtMoney(r.projectNet, curr));
        }
        ...
        if (els.projectHint) {
          if (s.proposalMode) {
            ...
            safeText(
              els.projectHint,
              projectOk
                ? `(${fmtNumber(s.projectHours, 0)}h × taxa/hora) com +${fmtNumber(s.scopeRisk, 1)}% margem de escopo e −${fmtNumber(s.discount, 1)}% desconto.`
                : "Preencha as horas para estimar."
            );
          }
        }

        let proposalBaseText = PLACEHOLDER;
        if (projectOk) {
          ...
          proposalBaseText = `Proposta comercial para execução do projeto conforme escopo e premissas acordadas. Investimento: ${fmtMoney(r.projectNet, curr)}. Prazo estimado: ~${fmtNumber(prazoDiasUteis, 0)} dias úteis.${roiLine} ${anti} ${negotiationCtx.roi.caveat}`;
        }
        let proposalFinalText = proposalBaseText;
        if (proposalJustificationPinned && !isPlaceholderText(proposalBaseText)) {
        ...
        if (strategistActive && computeStrategistMetrics && formatStrategistValue) {
          ...
          safeText(els.strategistVce, noPreco ? "—" : formatStrategistValue(strat.vce, "percent"));
          safeText(els.strategistVceLabel, noPreco ? "Preencha horas do projeto." : (strat.vceLabel || "—"));
          safeText(els.strategistRoix, noPreco ? "—" : (strat.roix != null ? `${fmtNumber(strat.roix, 1)}x` : "—"));
          safeText(els.strategistCdo, noPreco ? "—" : (strat.cdo != null ? fmtMoney(strat.cdo, curr) : "—"));
        ...
        if (els.configWrapper) {
          els.configWrapper.classList.toggle("hidden", isProposalMode && canComputeProposal);
        }
        ...
        if (els.resultCardsInternal) {
          els.resultCardsInternal.classList.toggle("hidden", isProposalMode);
        }
        if (els.resultCardProposal) {
          els.resultCardProposal.classList.toggle("hidden", !isProposalMode);
        }
        if (isProposalMode && els.resultProposalTotal && els.resultProposalPrazo) {
          if (canComputeProposal) {
            safeText(els.resultProposalTotal, fmtMoney(r.projectNet, s.currency));
          } else {
            safeText(els.resultProposalTotal, "Preencha as entradas essenciais para calcular o investimento.");
          }
          safeText(
            els.resultProposalPrazo,
            prazoDias != null ? `~${fmtNumber(prazoDias, 0)} dias úteis` : "-"
          );
        }
      ...
      function buildShareUrl(s, includePii, extraParams) {
        ...
        const piiKeys = ["professionalName", "clientName"];
        for (const k of QUERY_PARAM_KEYS) {
          if (piiKeys.includes(k) && !includePii) continue;
          params.set(k, String(sanitized[k] ?? ""));
        }
      ...
      function wireEvents() {
        ...
        document.addEventListener("click", (e) => {
          const btn = e.target?.closest?.(".discount-preset");
          if (!btn || !els.discount) return;
          const pct = Number(btn.getAttribute("data-discount-preset"));
          if (!Number.isFinite(pct) || pct < 0 || pct > 90) return;
          els.discount.value = String(pct);
          updateUI();
        });
      ...
      try {
        (function init() {
          ...
          if (!applied) {
            setInputsFromState(initialState);
          }
          currentUiMode = normalizeUiMode(initialState.uiMode);
          wireEvents();
          applyButtonHelp();
          wireHelpToggles();
          setupTabs();
